...
export default function RobotControls() {
  ...
  // Default aperto da md (>=768px). Su mobile parte chiuso per non coprire
  // il copy del hero. Resync se il media query cambia (rotazione device).
  useEffect(() => {
  ...
  return (
    <>
      ...
      <button
        ...
        className="absolute top-24 right-5 sm:right-6 z-40 flex items-center gap-2 bg-black/85 backdrop-blur-sm border border-white/15 text-white font-mono text-[10px] uppercase tracking-widest px-3 min-h-11 hover:border-acid hover:text-acid transition-colors"
      >
      ...
      {open && (
        <div
          ...
          className="
            absolute z-30 bg-black/90 backdrop-blur-sm border border-white/15
            text-white font-mono text-[11px] select-none shadow-2xl
            inset-x-4 bottom-4 max-h-[55vh] overflow-y-auto
            md:inset-x-auto md:bottom-auto md:top-40 md:right-6 md:w-64 md:max-h-none md:overflow-visible
          "
        >
          ...
          <details open className="group border-b border-white/10">
            ...
            <div className="px-3 pb-3 pt-1 flex items-center justify-between gap-3">
              ...
              <select
                id="robot-state"
                value={state}
                onChange={(e) => setState(e.target.value as RobotState)}
